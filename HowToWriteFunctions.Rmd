...
I've saved a data set (originally from GapMinder) that has information on HIV prevelance by country
over time. I've saved the data as an [Rds file]("HIVdata.Rds"),
so we can skip the clean up for now and focus on the plot

```{r, read-hiv, exercise = TRUE}
HIVdata <- readRDS("HIVdata.Rds")   # put this file in same folder as the Rmd file
head(HIVdata, 3)
```
...
Edit the cod below by adding one line at the top and one line at the bottom
...
```{r, first-function-solution, exercise = TRUE}
HIVplot <- function() {
  HIVdata %>%
    filter(country %in% c("Uganda", "Kenya", "Tanzania", "South Africa",
                          "Zimbabwe", "United States")) %>%
...
```{r, a-chunk, exercise.setup = "load-gapminder", exercise = TRUE}
hiv_url <- 
...
```{r, life-expectency, exercise.setup = "load-gapminder", exercise = TRUE}
le_url <-
...
```{r, two-plot-refine, exercise.setup = "two-plots-setup", exercise = TRUE}
