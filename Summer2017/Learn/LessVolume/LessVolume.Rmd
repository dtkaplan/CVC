...
So far we have focused on position, but we can also change things like
color, size, and shape by augmenting our formula.

 * Use terms of the form `attribute:variable` to determine an attribute
 based on values in a variable

```{r fig.width = 8, fig.height = 3, fig.align = "center", tidy = FALSE}
...
gf_freqpoly( ~ age + color:substance | sex, data = HELPrct, binwidth = 5)
```
...
```{r fig.show = 'hold', out.height = 200}
gf_histogram( ~ age, data = HELPrct, binwidth = 5 )
