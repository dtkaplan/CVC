...
Often it is useful to work with a smaller subst of the data until we get our 
...
Use `mutate()` or `transmute()` to create a new varaible.  How do the two differ?
...
Note: `summarize()` works for non New Zelanders, but it conflicts with a funciton with
...
Movies %>% 
  group_by(mpaa) %>% 
  summarise(avg_length = mean(hours, na.rm = TRUE) / 60) %>% 
  arrange(avg_length)
```
