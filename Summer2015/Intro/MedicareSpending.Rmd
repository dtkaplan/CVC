...
ResidsWithDemographics <- ProviderResids %>%
  inner_join(MedicareProviders) %>%
  inner_join(Zips, by=c(zipProvider = "ZIP")) %>%
  mutate(pop_density = Pop / LandArea, 
         college_educated = College / Pop)
mod2 <- lm(mresid ~ Income + Pop, data = ResidsWithDemographics)
             I(Pop/LandArea) + Income + I(College/Pop),
           data=ResidsWithDemographics )
rsquared(mod2)
anova(mod2)
mod3 <- lm(mresid ~ I(Pop/LandArea) + Income + I(College/Pop) + State + Latitude, 
           data=ResidsWithDemographics )
rsquared(mod3)
anova(mod3)
```
