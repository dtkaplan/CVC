...
require(lubridate)
Births <- Births78 %>% 
  mutate( date = mdy(date) - years(100),  # y2k fix
...
```{r, echo=FALSE}
ggplot(data=Births, aes(x=date, y=births)) + geom_point()
```
...
* Note that `colour = "navy"` is now outside of the aesthetics list.  That's how `ggplot2` distinguishes between mapping and setting.
 ...
 * aesthetics get computed twice, once before the stat and again after.   Example: histograms
...
require(xkcd)
qplot( x=date, y=births, data=Births, color=wday, 
